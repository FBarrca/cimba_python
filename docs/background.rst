...
.. note::
    ...
    This background section mirrors the upstream Cimba documentation because the
    Python package wraps the same native engine. Where the original text
    discusses C structs, C function names, and internal ``cmi_*`` building
    blocks, read those as implementation background unless the corresponding
    ``cimba.sim`` API is named explicitly. Python-only gaps are tracked in
...
The Python wrapper exposes a flatter modeling namespace, ``cimba.sim``. For
example, :c:func:`cmb_process_hold()` becomes ``sim.hold()``,
:c:func:`cmb_random_exponential()` becomes ``sim.exponential()``, and
:c:func:`cmb_buffer_put()` becomes ``sim.put()``. Model entities are declared as
annotated fields such as ``sim.Queue``, ``sim.Resource``, ``sim.Pool``,
``sim.Store``, ``sim.PQueues``, ``sim.Condition``, and ``sim.Dataset``.
...
In Python, the generated trial function initializes and terminates the native
event queue automatically. User models usually interact with events through
blocking process verbs like ``sim.hold()``, timers like ``sim.timer_add()``, and
``sim.wait_event()``. Direct arbitrary event scheduling is not exposed yet.
...
There is also a ``cmb_assert_always()`` that remains even if ``NASSERT`` is defined. This
...
wrappers for ``malloc()`` and its friends to test for out-of-memory conditions. These
...
The Python API exposes the same trial-local native pseudo-random number
generator through ``cimba.sim`` functions such as ``sim.random01()``,
``sim.exponential()``, ``sim.normal()``, ``sim.gamma()``, ``sim.pert()``,
``sim.poisson()``, ``sim.categorical()``, and many others. Native alias-table
objects are not exposed yet; use ``sim.categorical()`` or ``sim.loaded_dice()``
for weighted discrete sampling.
...
Python exposes ``sim.Dataset`` with ``sim.tally()``, ``sim.dataset_mean()``,
``sim.dataset_count()``, ``sim.dataset_min()``, ``sim.dataset_max()``, and
``sim.dataset_std()``. Time-weighted recording summaries are exposed for queues,
resources, pools, stores, and priority queues through functions such as
``sim.mean_level()`` and ``sim.mean_in_use()``. Raw native dataset, timeseries,
histogram, correlogram, and report-printing APIs are not exposed yet.
...
In Python, a ``sim.Model`` subclass and its registered process functions define
the trial function. ``model.experiment(...)`` creates the native trial array from
parameter values and replication counts, and ``Experiment.run()`` calls the
native multithreaded experiment runner.
...
The benchmark discussion below compares upstream native Cimba with SimPy. Cimba
Python uses the same native engine for event queues, processes, resources, and
random draws, while Python process bodies are compiled with Numba before they
are registered with the native engine. Exact performance depends on the model
body, Numba compilation, and the current wrapper overhead.
...
Very simple. This is a simulation library in C, the author's initials are 'AMB', and
'simba' means 'lion' in Swahili. Of course it had to be named Cimba. The Python
wrapper keeps the name because it still runs on the native Cimba engine.
