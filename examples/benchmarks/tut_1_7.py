...
class MM1Tutorial(sim.Model):
    arr_rate: sim.Param
    srv_rate: sim.Param
    warmup_time: sim.Param
    dur_s: sim.Param
...
mm1 = MM1Tutorial("mm1_tutorial")


@mm1.process
def arrival(env: MM1Tutorial):
...
@mm1.collect
def collect_stats(env: MM1Tutorial):
...
def run_experiment(
    *, n_reps: int, duration: float, warmup_time: float, seed: int | None
...
def print_report(rhos: np.ndarray, queue_lengths: np.ndarray, n_reps: int) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "-t", "--timing", action="store_true", help="Print wall-clock timing"
    )
    parser.add_argument(
        "-n", "--reps", type=int, default=10, help="Replications per rho (default 10)"
    )
    parser.add_argument(
        "-d",
        "--duration",
    ...
    print(
        f"M/M/1 tutorial 1.7 (cimba.sim): {N_RHOS} rho values × "
    ...
    elapsed, queue_lengths = run_experiment(
        n_reps=args.reps,
